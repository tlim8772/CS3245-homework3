...
def get_doclens_docfreqs_offsetdict(out_dict: str) -> tuple[dict[str, int], dict[str, int], dict[str, int]]:
...
def get_posting_list(token, offset_dict: dict[str, int], posting_file: IO[bytes]) -> list[tuple[str, float]]:
...
def eval(query: str, doc_len: dict[str, int], doc_freqs: dict[str, int], offset_dict: dict[str, int], posting_file: IO[bytes]) -> list[str]:
    docN = len(doc_len)
    stemmer = nltk.stem.PorterStemmer()
    doc_res: dict[str, float] = {}
    ...
    result = heapq.nlargest(10, doc_res.items(), key=lambda pair: (pair[1], -int(pair[0])))
